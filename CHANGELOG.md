...
- `get_admin_proposal_state` / `get_operator_proposal_state` — full pending proposal state for admin dashboards, exposing address + ledger timestamp + proposer (#97).
- `AdminProposalState` / `OperatorProposalState` contract types — structured pending-proposal records replacing the legacy `Address`-only storage (#97).
- `set_config` reward-margin enforcement — `penalty_per_minute * 1.5 < reward_base` checked on every canonical and custom severity update (`SLAError::RewardMarginTooLow`, #92).
- `set_config` cross-severity enforcement — `critical.penalty_per_minute >= high.penalty_per_minute` checked against live storage (`SLAError::SeverityOrderInvalid`, #92).
- Failure codes 19 (`RewardMarginTooLow`) and 20 (`SeverityOrderInvalid`) appended to `get_failure_schema` (#92).
- Storage version bumped 1 → 2 with v1 → v2 migration step that clears any leftover bare-Address pending proposal storage (#97).

### Changed

- Soroban SDK reference reconciled to 21.1.0 across README, `Cargo.toml`, project docs and the `apexchainx_calculator` crate top-of-file doc comment (#73).
- Validate pipeline reference in `RUST_TOOLCHAIN_PINNING.md` updated to 21.1.0 (#73).
### Changed
