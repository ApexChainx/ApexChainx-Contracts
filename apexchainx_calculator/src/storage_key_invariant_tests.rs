...
/// migration step added: `HISTORY_LEN_KEY` at v2 joins the v1 keys.
...
const SNAPSHOT_STORAGE_VERSION: u32 = 2;
...
fn test_storage_key_set_pinned_to_version_snapshot_or_newer() {
    ...
    let matches_snapshot = current.len() == KEYS_AT_V2.len()
        && KEYS_AT_V2.iter().all(|k| current.contains(k))
        && current.iter().all(|k| KEYS_AT_V2.contains(k));
