...
fn test_extreme_penalty_large_overtime_no_i128_overflow() {
    // Worst-case within the documented bound: low threshold=1, penalty=100,
    // mttr=MAX_MTTR. overtime = MAX_MTTR - 1 ≈ 5.26e5; penalty ≈ 5.26e7.
    ...
    let result = client2.calculate_sla_view(&symbol_short!("OVF"), &symbol_short!("low"), &MAX_MTTR);
    assert_eq!(result.status, symbol_short!("viol"));
    let expected = -((MAX_MTTR - 1) as i128 * 100);
